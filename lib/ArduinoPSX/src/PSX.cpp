...
void PSX::setRumble(byte smallMotor, byte largeMotor)
{
...
int PSX::read(PSXDATA &psxdata)
{
    ...
    {
        byte data[6];
        // Bytes 0-1: button data responses; cmd carries rumble on first two TX bytes of payload
        sendCommand(_rumbleSmall, data[0]);
        sendCommand(_rumbleLarge, data[1]);
        for(int i=2; i<6; i++) sendCommand(PSXPROT_IDLE, data[i]);
